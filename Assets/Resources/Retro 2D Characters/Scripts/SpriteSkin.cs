...
public class SpriteSkin : MonoBehaviour {
	...
	public string folderPath = "Retro 2D Characters/Spritesheets/";
	...
	void LateUpdate ()
	{
		...
		if (newSprite != null && spriteRenderer != null && newSpritesheet.Length > 0)
		{
			...
			//If we're using the color outline variations, offset the sprite frame index. Only relevant for base sprites.
			if (useColorOutline == true && isChildSprite == false)
			{
				frameIndex += 128;
			}
			//Finally, set the new sprite to render. also don't do this if it doesn't exist
            if(!(frameIndex>=newSpritesheet.Length))
